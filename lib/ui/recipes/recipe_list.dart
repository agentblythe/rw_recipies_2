...
class _RecipeListState extends State<RecipeList> {
  ...
  Future loadRecipes() async {
    ...
    // rootBundle is the top-level property that holds references
    // to all the items in the asset folder.
    // This loads the file as a string.
    final jsonString = await rootBundle.loadString('assets/recipes1.json');
    setState(() {
      // Uses the built-in jsonDecode() method to convert the string to a map,
      // then uses fromJson(), which was generated for you,
      // to make an instance of an APIRecipeQuery.
  ...
  Widget _buildRecipeLoader(BuildContext context) {
    // Checks to see if the list of recipes is null.
    if (_currentRecipes1 == null || _currentRecipes1?.hits == null) {
      return Container();
    }
    // Show a loading indicator while waiting for the recipes
    return Center(
      // If not, calls _buildRecipeCard() using the first item in the list.
  ...
  Widget _buildRecipeCard(
    ...
    return GestureDetector(
      ...
      // Calls recipeStringCard(), which shows a nice card below
      // the search field.
